...
In the mixture model , MAR holds `r citep(bib2[[c('molen1998','wang2011')]])` if and only if, for each $j \geq 2$ and $k < j$:
\[
...
`r citep(bib2[['minqa']])`. Denote $J(\mathbf \xi) = - \log L = - \log \sum_{i =
...
convergence using R version 2.15.3 (2013-03-01) `r citep(bib2[['R']])` and
...
*quantreg* R package `r citep(bib2[['quantreg']])` and Bottai's algorithm
`r citep(bib2[['bottai2013']])` (noted as BZ). The *rq* function
...
distributional assumptions. `r citet(bib2[['bottai2013']])` impute missing
...
management clinical trial `r citep(bib2[['perri2008extended']])`.  This trial was
...
conditional on random effects as in `r citet(bib2[['yuan2010']])`. In addition, our
...
`r citet(bib2[['bottai2013']])` if error is mis-specified. We also illustrated
...
each pattern; otherwise MAR constraints do not exist `r citep(bib2[['wang2011']])`.
