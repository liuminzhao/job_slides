...
- `r citet(bib[['walker1999']])`: diffuse finite Polya Tree
- `r citet(bib[['yu2001']])`: asymmetric Laplace distribution
- `r citet(bib[['hanson2002']])`: mixture of polya tree prior for median regression on survival time in AFT model
- `r citet(bib[['kottas2009']])`: semi-parametric models using DP
  mixtures for the error distribution
- `r citet(bib[['reich2010']])`: an infinite mixture of Gaussian densities for error
- `r citet(bib3[['kozumi2011']])`: developed a simple and efficient Gibbs
...
- Not coherent to pool from every individual quantile regression, because the sampling distribution of $Y$ for $\tau_1$
  ...
  under two different quantile regressions `r citep(bib[['tokdar2011']])`
...
- `r citet(bib[['berger2001']])` considered
...
- The rule of thumb for choosing $M$ is to set $M=\log_2n$, where $n$
  is the sample size `r citet(bib[['hanson2002']])`
...
- *RQ*: rq function in `r citep(bib[['quantreg']])`
(frequentist quantile regression method)
- *FBQR*: flexible Bayesian
quantile regression `r citep(bib[['reich2010']])`
...
- `r citet(bib2[['wei2012']])` proposed a multiple imputation method
...
- `r citet(bib2[['bottai2013']])` illustrated an imputation method
...
- `r citet(bib2[['yuan2010']])` introduced a fully parametric Bayesian
...
- In mixture models , MAR holds `r citep(bib2[[c('molen1998','wang2011')]])` if and only if, for each
$j \geq 2$ and $k < j$:
\[
...
of sensitivity parameters `r citep(bib2[['dh2008']])`, where $k < j, 2 \leq j \leq
J $.
...
sensitivity parameters at a manageable level `r citep(bib2[['dh2008']])` and
...
after lifestyle treatment `r citep(bib2[[c('wadden2001', 'perri2008extended')]])`, we
assume that
\[
