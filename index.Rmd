...
bib <- read.bibtex('qr-draft-reference.bib')
bib2 <- read.bibtex('qr-missing-reference.bib')
bib3 <- read.bibtex('qr-introduction.bib')
```
...
- R package `quantreg` `r citep(bib[['quantreg']])`
- Using simplex for linear programming problems mentioned in `r citet(bib[['koenker1978']])`
$$
...
- `r citet(bib[['walker1999']])`: diffuse finite Polya Tree in a generalized linear mixed model
- `r citet(bib[['yu2001']])`: asymmetric Laplace distribution (ALD) for QR under Bayesian framework
- `r citet(bib[['hanson2002']])`: mixture of Polya tree prior for median regression on survival time in AFT model
- `r citet(bib[['kottas2009']])`: semi-parametric QR models using mixtures of DP for the error distribution
- `r citet(bib[['reich2010']])`: an infinite mixture of two Gaussian densities for error
- `r citet(bib3[['kozumi2011']])`: developed a simple and efficient Gibbs sampling algorithm for fitting quantile regression based on a location-scale mixture representation of ALD
...
- Not coherent to pool from every individual quantile regression, because the sampling distribution of $Y$ for $\tau_1$ is usually different from that under quantile $\tau_2$ since they are assuming different error distribution under two different quantile regressions `r citep(bib[['tokdar2011']])`
...
Consider heterogeneous linear regression model from `r citet(bib[['he1998']])` :
...
- `r citet(bib[[c('freedman1963', 'fabius1964', 'ferguson1974')]])`, `r citet(bib[[c('lavine1992', 'lavine1994')]])`
...
<!-- - `r citet(bib[['ferguson1974']])` pointed out $\alpha_{\epsilon} = 1$ yields a $G$ that is absolutely continuous with probability 1 -->
<!-- - $\alpha_{\epsilon_1, \ldots, \epsilon_m} = m^2$ yields that $G$ is -->
<!-- absolutely continuous with probability 1. -->
<!-- - `r citet(bib[['walker1999']])` and -->
<!--  `r citet(bib[['paddock1999']])` considered $\alpha_{\epsilon_1, \ldots, -->
<!--   \epsilon_m} = cm^2$, where $c > 0$. -->

- `r citet(bib[['berger2001']])` considered $\alpha_{\epsilon_1, \ldots, \epsilon_m} = c \rho(m)$. In general, any $\rho(m) $ such that $\sum_{m=1}^{\infty} \rho(m)^{-1} < \infty$ guarantees $G$ to be absolutely continuous.
...
- The rule of thumb for choosing $M$ is to set $M=\log_2n$, where $n$ is the sample size `r citet(bib[['hanson2002']])`

<!-- - `r citet(bib[['hanson2002']])` showed the approximation -->
...
- Improve efficiency
- Use continuous spike and slab priors on each component of $(\mathbf \beta, \mathbf \gamma)$ `r citep(bib[['george1993']])`
...
<!-- - Adaptive Metropolis-Hasting algorithm -->
<!-- - $t_{\beta_j}, t_{\gamma_j}, t_{\sigma}, t_c$ are the tuning parameters to adjust acceptance rate `r citep(bib[['jara2009']])` -->

---
## Simulation

- *RQ*: rq function in `r citep(bib[['quantreg']])` (frequentist quantile regression method)
- *FBQR*: flexible Bayesian quantile regression `r citep(bib[['reich2010']])`
...
<!-- `r citet(bib[['reich2010']])`. Model 1H-4H (M1H-M4H) assume heterogeneous -->
...
<!-- \end{align*} -->
<!-- $$ -->
<!-- - $s_j = 1/1000$ `r citep(bib[['george1993']])` -->
...
<!-- - A recent weight management study `r citep(bib[['perri2008extended']])` -->
...
- `r citet(bib2[['wei2012']])` proposed a multiple imputation method for quantile regression model when there are some covariates missing at random (MAR).
...
- `r citet(bib2[['bottai2013']])` illustrated an imputation method using estimated conditional quantiles of missing outcomes given observed data.
...
- `r citet(bib2[['yuan2010']])` introduced a fully parametric Bayesian quantile regression approach for longitudinal data with non-ignorable missing data.
...
<!-- data in `r citet(bib2[['rubin1977']])`. -->
<!-- - Later mixture models were extended to handle MNAR in longitudinal -->
<!-- data. -->

<!-- - For discrete dropout times, -->
<!-- `r citet(bib2[[c('little1993','little1994')]])` proposed a general method -->
<!-- by introducing a finite mixture of multivariate distribution for -->
<!-- longitudinal data. -->

- When there are many possible dropout time, `r citet(bib2[['roy2003']])` proposed to group them by latent classes.
- `r citet(bib2[['roy2008']])` extended `r citet(bib2[['roy2003']])` to generalized linear models and proposed a pattern mixture model for data with non-ignorable dropout, borrowing ideas from `r citet(bib2[['heagerty1999']])`.
...
- In mixture models , MAR holds `r citep(bib2[[c('molen1998','wang2011')]])` if and only if, for each $j \geq 2$ and $k < j$:
$$
...
\end{align*}
$$
where $\mathbf \eta_{j-1}^{(k)} = \big( \eta_{y_1,j-1}^{(k)}, \ldots, \eta_{y_{j-1}, j-1}^{(k)} \big)$ for $k < j$. Then $\mathbf \xi_s = ( \mathbf h_j^{(k)}, \mathbf \eta_{j-1}^{(k)}, \delta_j^{(k)})$ is a set of sensitivity parameters `r citep(bib2[['dh2008']])`, where $k < j, 2 \leq j \leq J $.
...
<!-- - *rq* function (noted as RQ) in *quantreg* R package `r citep(bib2[['quantreg']])` -->
<!-- - Bottai's algorithm `r citep(bib2[['bottai2013']])` (noted as BZ). -->
<!-- - `r citet(bib2[['bottai2013']])` impute missing -->
...
- Based on previous studies of pattern of weight regain after lifestyle treatment `r citep(bib2[[c('wadden2001', 'perri2008extended')]])`, we assume that
$$
...
<!-- extreme quantiles comparing to `r citet(bib2[['bottai2013']])` if -->
